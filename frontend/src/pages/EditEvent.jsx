...
const EditEvent = () => {
    ...
    const fetchEvent = async () => {
      try {
        ...
        setFormData({
          ...
          price: data.price || 0,
          tickets: data.tickets && data.tickets.length > 0 ? data.tickets : [{ type: 'General Admission', price: 0, quantity: 100 }]
        });
      } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <div className="mb-12">
        <h1 className="text-4xl font-display font-bold text-white mb-4">Edit Event</h1>
        <p className="text-gray-400">Update the details for your event "{formData.title}".</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <button
          type="submit"
          disabled={saving}
          className="w-full py-4 bg-gradient-to-r from-primary to-secondary text-white font-bold rounded-xl hover:opacity-90 transition-opacity shadow-[0_0_20px_rgba(124,58,237,0.4)] disabled:opacity-50 flex items-center justify-center"
        >
          {saving ? <><Loader2 className="h-5 w-5 animate-spin mr-2" /> Saving Changes...</> : 'Save Changes'}
        </button>
      </form>
    </div>
  );
